...
void ProcessSelect(struct MyNODE *where, int N_Relations, int currRel);
MyNODE *be;
/*************************************************************************************************/
...
int parseSelect(char *current, char* inp, int n, char* token, int token_type) {
    ...
    current = peek(current, token, &token_type);
    if (token_type == WHERE) {
        printf("Error: at least one relation name is required...\n");
        return PARSE_ERROR;
    }
    ...
    if (token_type != IDENTIFIER) {
        printf("Error: %s, %s\n", token, "appropriate relation name is required..");
        return PARSE_ERROR;       
    }
    // scan over
    current = lex_anal(current, token, &token_type);
    ...
    while (current != inp + n || (token_type == WHERE || token_type == IDENTIFIER || token_type == ',')) {
        if (token_type == ',' && current == inp + n) {
        ...
        if (token_type == ',') {
            ...
            if (FILE *file = fopen(token, "r")) {
                fclose(file);
                
                // copy the relation name
                for (int i = 0; i < strlen(token); i++) {
                    selo->RelName[selo->NRels][i] = token[i];
                }   
                ...
                if (token_type == IDENTIFIER) {
                    ...
                    for (int i = 0; i < strlen(token); i++) {
                        selo->RelName[selo->NRels + 10][i] = token[i];
                    }
                    current = lex_anal(current, token, &token_type);
                    current = lex_anal(current, token, &token_type); 
                } else {
                ...
                printf("Error: %s, %s\n", token, "realation is not in the database...");
                return PARSE_ERROR;
            }                 
        } else {
            printf("Error: %s, %s\n", token, "appropriate relation name is required..");
    ...
    if (token_type != IDENTIFIER && token_type != ',' && token_type != WHERE && current != inp + n) {
        printf("Error: %s, %s\n", token, "appropriate relation name is required..");
        return PARSE_ERROR;       
    }
    ...
    if (token_type == '*') {
        ...
        selo->NAttrs--;
        printf("%s\n", "------------------------------------------------");
        ProcessSelect(selo->where, selo->NRels, 0);
        return NO_ERROR;
    }
...
void ProcessSelect(struct MyNODE *where, int N_Relations, int currRel) {
    if (currRel <= N_Relations) {
        ...
        while (selo->Rel[currRel]->GetNext() != 0) {
            ...
            ProcessSelect(where, N_Relations, currRel + 1);
        }
    } else {
        if (where != NULL) {
            int result = Eval(be);
      ...
      while ( current != inp + n ) { // p did not reach end of line 
       ...
       /* ========================================
        ...
        if (token_type == CREATE) { 
            ...
            err = parseSelect(current, inp, n, token, token_type);
        } else { 
